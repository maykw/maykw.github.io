function getBingoBoard(bingoList, size, options = {seed:"", mode:"normal", lang:"name"}) {
	...
	function difficulty(i) {
		...
		if (MODE == "short") { value = Math.floor(value/2); } // if short mode, limit difficulty
			else if (MODE == "long") { value = Math.floor((value + 2) / 2); }
            else if (MODE == "special") { value = Math.floor((value + 4) / 2); }
			value++;
		return value;
	}
...
var bingo = function() {
	...
	var results = $("#results");
	results.append ('<a href="'+qSeed+qMode+qEx+qBoard+'&lang=ch"><img src="img/cn.png" alt="Chinese"></a><a href="'+qSeed+qMode+qEx+qBoard+'"><img src="img/en.png" alt="English"></a><a href="'+qSeed+qMode+qEx+qBoard+'&lang=jp"><img src="img/jp.png" alt="Japanese"></a><p>BotW Bingo <strong>v1</strong>&emsp;Seed: <strong>' +
